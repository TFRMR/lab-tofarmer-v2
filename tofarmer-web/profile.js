...
async function loadNotifikasiUser(reset = true) {
  ...
  if (reset) { notifPage = 0; notifHabis = false; }
  ...
  try {
    const [
      ...
      supabaseClient.from("notifications").select("*").eq("user_id", currentWallet).order("created_at", { ascending: false }).range(from, to),
      supabaseClient.from("comments").select("id, post_id, user_id, comment, created_at").neq("user_id", currentWallet).order("created_at", { ascending: false }).range(from, to),
      supabaseClient.from("contributions").select("id, user_id, judul_aksi, created_at").neq("user_id", currentWallet).order("created_at", { ascending: false }).range(from, to),
      supabaseClient.from("ilmu_baku").select("id, user_id, judul_aksi, approved_at").neq("user_id", currentWallet).order("approved_at", { ascending: false }).range(from, to),
      supabaseClient.from("ilmu_pending").select("id, user_id, judul_aksi, created_at").neq("user_id", currentWallet).order("created_at", { ascending: false }).range(from, to),
      supabaseClient.from("reactions").select("id, post_id, user_id, type, created_at").neq("user_id", currentWallet).order("created_at", { ascending: false }).range(from, to),
    ]);
